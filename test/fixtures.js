export async function seed(
  ...
  await db.execute({
    sql: `INSERT INTO messages (id,conversation_id,inbox_address,message_id,dedupe_key,direction,from_email,from_name,envelope_from,to_email,to_header,cc,subject,text_body,html_body,received_at,created_at,reply_to)
    VALUES (?,?,?,?,?,'inbound','customer@example.net','Jane Customer','bounce@example.net',?,'[]','[]',?,?,?,?,?,?)`,
    args: [
      `m-${id}`,
      id,
      address,
